class Listado {
    ...
    buscarRestaurante(id) {
        var listado = this.restaurantes.find(restaurantes => restaurantes.id === id);
        return listado ? listado : "No se ha encontrado ningún restaurant";
        // if (listado) {
        //     return listado;
        // } else {
    ...
    obtenerCiudades() {
        ...
        var ciudades = this.restaurantes.map(restaurantes => restaurantes.ubicacion);
        var ciudadesFiltradas = this.filtrarRepetidos(ciudades);
    ...
    obtenerRubros() {
        var rubros = this.restaurantes.map(restaurantes => restaurantes.rubro);
    ...
    obtenerHorarios() {
        ...
        var arregloHorarios = this.restaurantes.map(restaurantes => restaurantes.horarios);
        //En este arreglo vamos a poner todos los horarios, uno por uno
        var horarios = [];
        arregloHorarios.forEach(function (horarioArray) {
            horarioArray.forEach(function (hora) {
                horarios.push(hora);
            });
        });
    ...
    //funcion generica para filtrar elementos repetidos en un array
    filtrarRepetidos(array) {
        var filtrados = array.filter((elem, index, self) => index === self.indexOf(elem));
        return filtrados;
    }
}
